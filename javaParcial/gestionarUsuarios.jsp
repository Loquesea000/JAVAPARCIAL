...
<div class="tab-pane fade" id="docentes" role="tabpanel" aria-labelledby="docentes-tab">
    <sql:query var="docentes" dataSource="${DataSource}">
        SELECT u.cedula, u.nombre, u.telefono, u.correo, u.contra, u.fecha_registro, d.id_cargo
        FROM usuarios u
        JOIN rol_usuario r ON u.cedula = r.cedula
        JOIN docentes d ON u.cedula = d.cedula
    ...
    <table class="table table-bordered">
        ...
        <c:if test="${not empty docentes.rows}">
            <c:forEach var="row" items="${docentes.rows}">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${row.id_cargo == 1}">
                                DIRECTOR
                            </c:when>
                            <c:when test="${row.id_cargo == 2}">
                                EVALUADOR
                            </c:when>
                            <c:otherwise>
                                SIN ASIGNAR
        ...
        <c:if test="${empty docentes.rows}">
            <tr>
                <td colspan="8" class="text-danger text-center">No hay docentes registrados.</td>
            </tr>
        </c:if>
        </tbody>
    </table>
