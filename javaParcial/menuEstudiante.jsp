...
<style>
        ...
        .table-custom {
    width: 10%; /* Cambia el porcentaje según sea necesario */
...
<div class="container">
    <h1 class="text-center mb-5 mt-5">Menú Estudiante</h1>

    <!-- Almacenar cedulaEstudiante en la sesión si está en el request -->
    <c:if test="${not empty param.cedulaEstudiante}">
        <c:set var="cedulaEstudiante" value="${param.cedulaEstudiante}" scope="session" />
    </c:if>

    <!-- Si no hay cedulaEstudiante en el request, tomarla de la sesión -->
    ...
    <c:choose>
        <c:when test="${not empty cedulaEstudiante}">
           ...
           <!-- Cards de Acciones -->
            ...
            <!-- Muestra la tabla de anteproyectos -->
             <h2 class=" mb-5 mt-5">Anteproyectos Asociados</h2>
                            ...
                            <td class="action-buttons">
                                <c:choose>
                                    <c:when test="${anteproyecto.est != 0}">
                                        <c:choose>
                                            <c:when test="${not empty anteproyecto.URL}">
                                                <a href="cargarDocumento.jsp?idAnteproyecto=${anteproyecto.id_anteproyecto}" class="btn btn-warning">
                                                    <i class="fas fa-edit"></i> Corregir Documento
                                                </a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="cargarDocumento.jsp?idAnteproyecto=${anteproyecto.id_anteproyecto}&cedulaEstudiante=${cedulaEstudiante}" class="btn btn-info">
                                    ...
                                    <c:otherwise>
                                        <!-- La celda quedará vacía aquí, sin contenido ni botones -->
                                    </c:otherwise>
                                </c:choose>
                            </td>
                        </tr>
            ...
            <c:if test="${empty anteproyectos.rows}">
                <p>No hay anteproyectos asociados a este estudiante.</p>
            </c:if>
        </c:when>
        <c:otherwise>
